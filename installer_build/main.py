...
class LoginWindow:
    ...
    def _check_and_run_first_setup(self):
        """Check if first-run setup is needed and run if necessary"""
        import sys
        config_file = self._get_config_file()
        
        # Check if config.ini exists and has server database path
        server_db_path = r"\\GVBSERVER\C$\Program Files\TASMA Board Room Booking System\bookings.db"
        ...
        # If no server config, show setup wizard
        if not has_server_config:
            self._show_first_run_setup(server_db_path, config_file)
